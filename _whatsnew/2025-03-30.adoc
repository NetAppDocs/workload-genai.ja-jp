...
Workload Factory API を使用すると、汎用 NFSv3、NFSv4、または SMB 共有からデータ ソースを追加できるようになりました。  NFS または SMB 共有からデータソースを追加すると、ナレッジベースボリュームはAmazon FSx for NetApp ONTAPボリュームに残ります。  Workload Factory Web UI は、将来のリリースでこの機能をサポートする予定です。

link:https://console.workloads.netapp.com/api-doc["ワークロードファクトリーAPIを使用する"^]
